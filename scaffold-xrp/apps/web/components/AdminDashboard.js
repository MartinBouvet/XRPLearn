        ...
        <div className="flex flex-col h-screen bg-gray-900 text-white font-sans overflow-hidden">
            ...
            <header className="bg-gray-800 border-b border-gray-700 p-4 flex justify-between items-center shadow-lg z-10">
                <div className="flex items-center gap-4">
                    <h1 className="text-2xl font-bold text-white tracking-wider">XRPL ADMIN DASHBOARD</h1>
                </div>
                        ...
                        {members.map((member, idx) => (
                            ...
                            >
                                ...
                                <div className="flex flex-col items-center text-center">
                                    ...
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold border mb-4 ${member.level == "3" ? "bg-green-900/30 text-green-400 border-green-500/30" :
                                        member.level == "2" ? "bg-purple-900/30 text-purple-400 border-purple-500/30" :
                                            "bg-blue-900/30 text-blue-400 border-blue-500/30"
                                        }`}>
                                        Level {member.level}
                                    </span>

                                    <div className="w-full bg-black/30 rounded p-2 mb-2">
                                        <div className="text-xs text-gray-500 uppercase font-bold">Balance</div>
                                        <div className="text-lg font-mono text-yellow-400">{member.balance ? `${member.balance} XRP` : "Loading..."}</div>
                                    </div>

                                    {/* Wallet Address */}
                                    <div className="bg-black/30 rounded px-2 py-1 text-[10px] font-mono text-gray-400 w-full truncate select-all cursor-pointer hover:text-white transition-colors" title={member.address}>
                                        {member.address}
                                    </div>
                                </div>
                            </div>
            ...
            {/* --- FOOTER --- */}
            <footer className="bg-gray-800 border-t border-gray-700 p-4 flex items-center justify-between text-sm">
