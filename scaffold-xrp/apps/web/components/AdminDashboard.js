    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white font-sans overflow-hidden transition-colors duration-300">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <aside className="w-96 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex flex-col shadow-2xl z-20 transition-colors duration-300">
                    <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                        <h2 className="text-lg font-bold flex items-center gap-2">
                            <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
                            LIVE LEDGER
                        </h2>
                        <p className="text-xs text-gray-500 dark:text-gray-400">Real-time transactions from students</p>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        {transactions.length === 0 ? (
                            ...
                            transactions.map((tx) => (
                                <div key={tx.id} className="bg-gray-50 dark:bg-gray-900 p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm animate-fade-in-left">
                                    <div className="flex justify-between items-start mb-2">
                                        <span className={`font-bold text-xs px-2 py-0.5 rounded ${tx.result === "tesSUCCESS" ? "bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" : "bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400"
                                            }`}>
                                            {tx.type}
                                        </span>
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-mono text-blue-500 truncate w-32 text-right" title={tx.from}>
                                                {getName(tx.from)}
                                            </span>
                                        </div>
                                        {tx.to && (
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-mono text-purple-500 truncate w-32 text-right" title={tx.to}>
                                                    {getName(tx.to)}
                                                </span>
                                            </div>
                                        )}
                                        <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700 flex justify-between font-bold">
                                            <span>Amount:</span>
                                            <span className="text-yellow-600 dark:text-yellow-400">{tx.amount} XRP</span>
                                        </div>
                                    </div>
                                </div>
                            ))
